...
### Situation  
The company was undergoing a **security improvement** initiative and required the team to add a **second encryption layer** beyond **HTTPS** to prevent **data exposure** on **rooted devices**, **emulators**, and **man in the middle attacks** for **confidential budget approvals**.

### Task  
Design and implement a second layer of **hybrid encryption** across **React Native** and **Java** to protect **confidential budget approvals** beyond **HTTPS**. Ensure **key management**, **payload encryption**, and **compatibility** with existing endpoints. Meet **security compliance** with **minimal latency** and **no UX disruption**, and deliver **auditable logs** with **rotatable keys**.  

### Actions  
I built a **hybrid encryption layer** to protect all communication between **React Native** and **Java APIs**. Instead of relying solely on HTTPS, I implemented an **RSA + AES model**, where each request was encrypted on the client and decrypted on the server using a **temporary session key**.  
...
This approach provided **end-to-end confidentiality** without affecting user experience. It was fully transparent to developers, maintained minimal overhead, and passed the company’s **security compliance audit** as a permanent component of the architecture.  
...
### Actions  
I built **React Native String Style**, an internal styling tool inspired by **Tailwind CSS**, to replace React Native Paper and remove dependency on rigid components. The system allowed developers to write **utility-based class strings directly in JSX**, merging structure and styling inside the same file for faster development. [This is the GitHub repository](https://github.com/ribas89/react-native-string-style).  

I refactored existing screens to adopt this syntax, simplifying layout creation and removing the need for complex StyleSheet files. I also introduced a **theme token layer** for colors, spacing, and typography, enabling quick global updates whenever the design system changed.  
...
### Situation  
The app was initially built entirely around **Redux**, which made even simple **UI** and **navigation states** cumbersome to manage. This led to **bloated reducers**, **tight coupling**, and reduced **scalability**.  
...
### Actions
My first step was to **analyze how state was shared** across screens and modules. I found that **Redux was overloaded**, handling everything from **user sessions** to **temporary UI flags**. I started by **separating concerns** and creating clear **boundaries between global, local, and transient state**. I introduced **React Context** for feature specific flows and **navigation parameters** for screen data transfers. To keep the team aligned, I **documented the new conventions**, built **typed navigation helpers**, and refactored existing modules to follow the new layered structure.

Below is an example showing how a screen combines **Redux**, **Context**, and **navigation parameters** to manage both global and local state in a cohesive way.
```tsx
...
export const ProfileScreen = () => {
  ...
  const { biometricsEnabled, userInfo } = (route?.params || {}) as any;

  const appContext = useContext(AppContext);

  const profile = useSelector(state => state.authReducer.profile);

  const [biometricEnabled, setBiometricEnabled] = useState(biometricsEnabled);
