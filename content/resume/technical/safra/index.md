...
### Frontend Engineer - AngularJS 📅 Jun 2019 – Aug 2020  
...
Below is a simplified diagram of the branching and release model that illustrates how the pipeline operated after these changes.  
...
2. **Consistent environments**: cascading resets kept develop, homolog, and preprod **always matched with production** after each release.  
3. **Cross-team stability**: the new model **cut merge conflicts from** around **15** per release to **just 2**.
...
6. **Predictable releases**: flexible release candidates included **only main commits and approved features**, ensuring **stable builds** from preprod to main.  
...
### Action  
I **joined the design squad** to understand the centralized work they were developing and **took responsibility** for bridging communication **between designers and developers**. After assessing each team's workflow, I determined that the **most effective way** to standardize the UI was by creating a **framework-agnostic, ready-to-use CSS** component library instead of AngularJS components, allowing squads to **use it in any context**. I built the library **with Sass** to ensure consistent styling and **documented** every component **using KSS** to provide clear visual references. To **distribute** it within the **restricted network**, I set up a **bare Git repository** on a shared internal resource, enabling **all squads to pull updates directly** into their projects.
